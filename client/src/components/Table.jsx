  ...
  if (tableName === "menuitem") {
    filteredData = data.map(item => {
      const newItem = Object.keys(item)
        .filter(key => key !== "restaurantID")
        .reduce((obj, key) => {
          obj[key] = item[key];
          return obj;
        }, {});
      newItem.originalRow = item;  // Store original row
      return newItem;
    });
  } else {
    filteredData = data.map(item => ({ ...item, originalRow: item }));
  }
        ...
        <tbody className="[&_tr:last-child]:border-0">
          {currentItems.map((row, index) => (
            <tr
              ...
              onClick={() => directPage(navigate, tableName, row)}
            >
              {Object.keys(filteredData[0])
                .filter(key => key !== "originalRow")
                .map(header => (
